...
HAMMING = ARGS = IMAGE_URIS = HASHES = HASH_DATA = FEATURE = None
...
def similar_memes(name):
    ...
    out = {'similar': [(int(dist), IMAGE_URIS[ind]) for dist, ind in HAMMING.knn(HASHES, h, 100)]}
    if ext == 'js':
        return out
    return ''.join('<img src="%s"/>' % x for _, x in out['similar'])
    

def main():
    global HAMMING, ARGS, IMAGE_URIS, HASHES, HASH_DATA, FEATURE
